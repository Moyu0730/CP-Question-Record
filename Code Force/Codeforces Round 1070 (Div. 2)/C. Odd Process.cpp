/* Question : Codeforces Round 1070 (Div. 2) - C. Odd Process */
...
inline void solve(){
    ...
    if( odd.sz > 0 ){
        ...
        for( int i = even.sz + 2 ; i <= n ; ++i ){
            if( i == n && odd.sz % 2 == 0 ) res = 0;
            else if( even.sz > 0 ){
                if( (i - (even.sz + 1)) % 2 == 0 ) res += even.back();
                else res -= even.back();
            }else{
                if( (i - (even.sz + 1)) % 2 == 0 ) res = odd[0];
                else res = 0;
            }
