...
inline void solve(){
    while( cin >> n ){
        ...
        l = r = -1;
        for( int i = 0 ; i < n ; ++i ){
            int idx = lower_bound(arr + i + 1, arr + n, sum - arr[i]) - arr;

            if( arr[idx] == sum - arr[i] && i < idx && ( l == -1 || r == -1 || arr[idx] - arr[i] < arr[r] - arr[l] ) ){
                l = i; r = idx;
            }
        }
        
        cout << "Peter should buy books whose prices are " << arr[l] << " and " << arr[r] << ".\n\n";
    }
}
