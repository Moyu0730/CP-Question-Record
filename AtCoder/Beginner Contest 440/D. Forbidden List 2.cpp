/* Question : AtCoder Beginner Contest 440 - D. Forbidden List 2 */
...
inline void solve(){
    ...
    for( int i = 1 ; i < n ; ++i ) pfs[i] = pfs[i-1] + arr[i] - arr[i-1] - 1;

    while( q-- ){
        cin >> x >> y;

        idx = LB(arr, x) - arr.begin();

        if( idx == arr.sz ){
            if( x == arr[idx-1] ) cout << x + y << "\n";
            else cout << x + y - 1 << "\n";
        ...
        sum = y;
        sum += pfs[idx];
        idx = LB(pfs, pfs[idx] + y) - pfs.begin();
        sum -= pfs[idx-1];
